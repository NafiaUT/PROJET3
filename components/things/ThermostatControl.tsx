    ...
    <div className="space-y-4">
      <div className="text-center">
        <div className="text-4xl font-bold text-gray-800 dark:text-gray-200">
          {thing.currentTemperature.toFixed(1)}&deg;C
        </div>
        <div className="text-sm text-gray-500 dark:text-gray-400">Temperature actuelle</div>
      </div>
      
      <div className="space-y-2">
        <label htmlFor={`target-temp-${thing.id}`} className="text-gray-500 dark:text-gray-400 text-sm">
          Target Temperature: {thing.targetTemperature}&deg;C
        </label>
        <input
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {Object.values(ThermostatMode).map((mode) => (
            ...
            >
              {mode === ThermostatMode.HEATING ? 'Chauffage' : mode === ThermostatMode.ECO ? 'ECO' : 'ETEINT'}
            </button>
          ))}
        </div>
      </div>
